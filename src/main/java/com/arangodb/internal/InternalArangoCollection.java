...
extends ArangoExecuteable<E> {
	...
	protected <T> Request insertDocumentRequest(final T value, final DocumentCreateOptions options) {
		final Request request = request(this.db.name(), RequestType.POST, PATH_API_DOCUMENT, this.name);
	...
	protected Request importDocumentsRequest(final String values, final DocumentImportOptions options) {
		...
		return importDocumentsRequest(options).putQueryParam("type", ImportType.auto).setJsonBody(values);
	}

	protected Request importDocumentsRequest(final VPackSlice values, final DocumentImportOptions options) {
		return importDocumentsRequest(options).putQueryParam("type", ImportType.auto).setBody(values);
	}
	...
	protected Request importDocumentsRequest(final DocumentImportOptions options) {
		...
		return request(this.db.name(), RequestType.POST, PATH_API_IMPORT).putQueryParam(COLLECTION, this.name)
	...
	protected Request getDocumentRequest(final String key, final DocumentReadOptions options) {
		final Request request = request(this.db.name(), RequestType.GET, PATH_API_DOCUMENT,
				DocumentUtil.createDocumentHandle(this.name, key));
	...
	protected Request getDocumentsRequest(final Collection<String> keys, final DocumentReadOptions options) {
		final DocumentReadOptions params = (options != null ? options : new DocumentReadOptions());
		final Request request = request(this.db.name(), RequestType.PUT, PATH_API_DOCUMENT, this.name)
	...
	protected <T> Request replaceDocumentRequest(
		...
		final Request request = request(this.db.name(), RequestType.PUT, PATH_API_DOCUMENT,
				DocumentUtil.createDocumentHandle(this.name, key));
	...
	protected <T> Request updateDocumentRequest(final String key, final T value, final DocumentUpdateOptions options) {
		final Request request = request(this.db.name(), RequestType.PATCH, PATH_API_DOCUMENT,
				DocumentUtil.createDocumentHandle(this.name, key));
	...
	protected <T> Request updateDocumentsRequest(final Collection<T> values, final DocumentUpdateOptions params) {
		final Request request = request(this.db.name(), RequestType.PATCH, PATH_API_DOCUMENT, this.name);
	...
	protected Request deleteDocumentRequest(final String key, final DocumentDeleteOptions options) {
		final Request request = request(this.db.name(), RequestType.DELETE, PATH_API_DOCUMENT,
				DocumentUtil.createDocumentHandle(this.name, key));
	...
	protected <T> Request deleteDocumentsRequest(final Collection<T> keys, final DocumentDeleteOptions options) {
		final Request request = request(this.db.name(), RequestType.DELETE, PATH_API_DOCUMENT, this.name);
		final DocumentDeleteOptions params = (options != null ? options : new DocumentDeleteOptions());
		request.putHeaderParam(TRANSACTION_ID, params.getStreamTransactionId());
		request.putQueryParam(ArangoRequestParam.WAIT_FOR_SYNC, params.getWaitForSync());
		request.putQueryParam(RETURN_OLD, params.getReturnOld());
	...
	protected Request documentExistsRequest(final String key, final DocumentExistsOptions options) {
		final Request request = request(this.db.name(), RequestType.HEAD, PATH_API_DOCUMENT,
				DocumentUtil.createDocumentHandle(this.name, key));
	...
	protected Request getIndexRequest(final String id) {
		return request(this.db.name(), RequestType.GET, PATH_API_INDEX, createIndexId(id));
	}

	protected Request deleteIndexRequest(final String id) {
		return request(this.db.name(), RequestType.DELETE, PATH_API_INDEX, createIndexId(id));
	}
	...
	protected Request createHashIndexRequest(final Iterable<String> fields, final HashIndexOptions options) {
		final Request request = request(this.db.name(), RequestType.POST, PATH_API_INDEX);
	...
	protected Request getIndexesRequest() {
		final Request request = request(this.db.name(), RequestType.GET, PATH_API_INDEX);
		request.putQueryParam(COLLECTION, this.name);
		return request;
	}
	...
	protected Request truncateRequest(final CollectionTruncateOptions options) {
		final Request request = request(this.db.name(), RequestType.PUT, PATH_API_COLLECTION, this.name, "truncate");
	...
	protected Request countRequest(final CollectionCountOptions options) {
		final Request request = request(this.db.name(), RequestType.GET, PATH_API_COLLECTION, this.name, "count");
	...
	protected Request dropRequest(final Boolean isSystem) {
		return request(this.db.name(), RequestType.DELETE, PATH_API_COLLECTION, this.name).putQueryParam("isSystem", isSystem);
	}
	...
	protected Request changePropertiesRequest(final CollectionPropertiesOptions options) {
		final Request request = request(this.db.name(), RequestType.PUT, PATH_API_COLLECTION, this.name, "properties");
	...
	protected Request renameRequest(final String newName) {
		final Request request = request(this.db.name(), RequestType.PUT, PATH_API_COLLECTION, this.name, "rename");
	...
	protected <T> Request responsibleShardRequest(final T value) {
		final Request request = request(this.db.name(), RequestType.PUT, PATH_API_COLLECTION, this.name, "responsibleShard");
		request.setBody(util(Serializer.CUSTOM).serialize(value));
		return request;
	}
	...
	protected Request grantAccessRequest(final String user, final Permissions permissions) {
		return request(ArangoRequestParam.SYSTEM, RequestType.PUT, PATH_API_USER, user, ArangoRequestParam.DATABASE,
				this.db.name(), this.name).setBody(util().serialize(OptionsBuilder.build(new UserAccessOptions(), permissions)));
	}
